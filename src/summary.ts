...
function formattingDescription(options: ImportSummaryOptions): string {
  const links = [
    options.linkMatchingVaultNotes ? "vault-note links on (attendee/title phrases)" : "vault-note links off",
...
export function summarizeImportOutcome(
  ...
  if (!options) {
    ...
    if (linkCount === 0) {
      return `Imported ${eventCount} Calendar ${eventLabel} into ${noteName}. No uniquely matched attendee names or title phrases appeared in event titles.`;
    }
    const linkLabel = linkCount === 1 ? "link" : "links";
    return `Imported ${eventCount} Calendar ${eventLabel} into ${noteName} and added ${linkCount} vault ${linkLabel} for matched attendee names or title phrases.`;
  }
  ...
  const linkSummary = linkCount === 0
    ? options.linkMatchingVaultNotes
      ? "No uniquely matched attendee names or title phrases appeared in event titles."
      : "Vault-note matching links are disabled."
    : `Added ${linkCount} vault ${linkLabel} for matched attendee names or title phrases.`;
